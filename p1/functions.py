...
def generate_single_atoms_p1(atom_amount, cell):
    atom_positions = np.zeros((atom_amount, 3))
    for i in range(atom_amount):
        for j in range(3):
            atom_positions[i][j] = round(random.uniform(0, 1), 3)
...
def sturcture_update_p1(positions, rigid_no, cell):
    rigid = positions.copy()
    # find the right rigid image
    for n in range(rigid_no):
        refrence = rigid[n]
        for i in range(rigid_no + 4*n, rigid_no + 4 + 4*n):
            image = AtomImage(rigid[i], cell)
            right_image = image.close_image_position(refrence)
            rigid[i] = right_image
    return rigid
...
def write_poscar_p1(positions, rigid_no, sim_box):
    with open('POSCAR', 'w') as poscar:
        poscar.write('Good luck\n')
        poscar.write('1.0\n')
        for i in range(3):
            poscar.write(str(sim_box[i][0]) + '  ' + str(sim_box[i][1]) + '  ' + str(sim_box[i][2]) + '\n')
        poscar.write('P' + '  ' + 'S' + '  ' + 'Li' + '\n')
        poscar.write(f'{rigid_no}  {rigid_no * 4}  {rigid_no * 3}' + '\n')
        poscar.write('Car\n')
        positions_len = len(positions)
        for i in range(positions_len):
            x = round(positions[i][0], 10)
...
def rigid_poscar_transform_PS4(positions, rigid_no, mode):
    # RtoP: rigid to poscar; PtoR poscar to rigid
    atom_in_rigid = 5
    type_1 = np.zeros((rigid_no * 1, 3))
    # type_2 = np.zeros((rigid_no * 4, 3))
    single_atom = positions[rigid_no * atom_in_rigid:]
    # rigid_array = np.zeros((atom_in_rigid * rigid_no, 3))
    if mode == 'RtoP':
        for i in range(rigid_no):
            type_1[i] = positions[i * 5]
        type_2 = positions[1:5]
        for i in range(1, rigid_no):
            inter = positions[1 + i*atom_in_rigid:5 + i*atom_in_rigid]
            type_2 = np.concatenate((type_2, inter))
        poscar_position_1 = np.concatenate((type_1, type_2))
        position_final = np.concatenate((poscar_position_1, single_atom))
    if mode == 'PtoR':
        rigid_array = np.concatenate((positions[0:1], positions[rigid_no:4+rigid_no]))
        for i in range(1, rigid_no):
            rigid_inter = np.concatenate((positions[i:i+1], positions[rigid_no + i*4:4+(rigid_no + i*4)]))
            rigid_array = np.concatenate((rigid_array, rigid_inter))
        position_final = np.concatenate((rigid_array, single_atom))
...
def setup_file_copy_p1(my_cwd, sym_no):
    shutil.copy('INCAR', my_cwd + f'/sym{sym_no}' + '/trail')
    shutil.copy('POSCAR', my_cwd + f'/sym{sym_no}' + '/trail')
    shutil.copy('POTCAR', my_cwd + f'/sym{sym_no}' + '/trail')
    shutil.copy('KPOINTS', my_cwd + f'/sym{sym_no}' + '/trail')
    shutil.copy('vasp.sub', my_cwd + f'/sym{sym_no}' + '/trail')

    shutil.copy('INCAR_relax', my_cwd + f'/sym{sym_no}' + '/relax' + '/INCAR')
    shutil.copy('POSCAR', my_cwd + f'/sym{sym_no}' + '/relax')
    shutil.copy('POTCAR', my_cwd + f'/sym{sym_no}' + '/relax')
    shutil.copy('KPOINTS_relax', my_cwd + f'/sym{sym_no}' + '/relax' + '/KPOINTS')
    shutil.copy('vasp.sub', my_cwd + f'/sym{sym_no}' + '/relax')

    shutil.copy('functions.py', my_cwd + f'/sym{sym_no}')
    shutil.copy('interprocess.py', my_cwd + f'/sym{sym_no}')
    shutil.copy('mc.py', my_cwd + f'/sym{sym_no}')
    shutil.copy('metropolis.py', my_cwd + f'/sym{sym_no}')
    shutil.copy('rigid.py', my_cwd + f'/sym{sym_no}')
    shutil.copy('symdata.py', my_cwd + f'/sym{sym_no}')
    
    shutil.copy('INPUT', my_cwd + f'/sym{sym_no}')
    shutil.copy('structureupdate.py', my_cwd + f'/sym{sym_no}')
    shutil.copy('vasp2.sub', my_cwd + f'/sym{sym_no}')
    shutil.copy('readinginput.py', my_cwd + f'/sym{sym_no}')
    shutil.copy('randommove.py', my_cwd + f'/sym{sym_no}')